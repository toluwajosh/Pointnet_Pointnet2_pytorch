...
def recognize_all_data(test_area=5):
    ...
    room_filelist = [
        line.rstrip()
        for line in open("./data/indoor3d_sem_seg_hdf5_data/room_filelist.txt")
    ]
    data_batch_list = []
    label_batch_list = []
...
class S3DISDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(10):
            ...
            curchoice = (
                np.sum(
                    (point_set[:, 0:3] >= (curmin - 0.2))
                    * (point_set[:, 0:3] <= (curmax + 0.2)),
                    axis=1,
                )
                == 3
            )
            ...
            mask = (
                np.sum(
                    (cur_point_set >= (curmin - self.padding))
                    * (cur_point_set <= (curmax + self.padding)),
                    axis=1,
                )
                == 3
            )
...
class S3DISDatasetWholeScene:
    ...
    def __getitem__(self, index):
        ...
        nsubvolume_x = np.ceil((coordmax[0] - coordmin[0]) / self.block_size).astype(
            np.int32
        )
        nsubvolume_y = np.ceil((coordmax[1] - coordmin[1]) / self.block_size).astype(
            np.int32
        )
        point_sets = list()
        semantic_segs = list()
        ...
        for i in range(nsubvolume_x):
            for j in range(nsubvolume_y):
                ...
                curchoice = (
                    np.sum(
                        (point_set_ini[:, 0:3] >= (curmin - 0.2))
                        * (point_set_ini[:, 0:3] <= (curmax + 0.2)),
                        axis=1,
                    )
                    == 3
                )
    ...
    def __getitem__(self, index):
        ...
        for i in range(nsubvolume_x):
            for j in range(nsubvolume_y):
                ...
                mask = (
                    np.sum(
                        (cur_point_set[:, 0:3] >= (curmin - self.padding))
                        * (cur_point_set[:, 0:3] <= (curmax + self.padding)),
                        axis=1,
                    )
                    == 3
                )
        ...
        for block_idx in range(num_blocks):
            ...
            if point_idx_block.shape[0] % self.block_points != 0:
                makeup_num = (
                    self.block_points - point_idx_block.shape[0] % self.block_points
                )
            ...
            div_blocks_center += [
                block_center[block_idx].copy() for i in range(len(sub_blocks))
            ]
