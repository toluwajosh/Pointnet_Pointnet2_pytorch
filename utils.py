    ...
    for j in range(pred.size(0)):
        ...
        for cat in np.unique(batch_target):
            ...
            I = np.sum(np.logical_and(
                batch_choice == cat, batch_target == cat))
...
def compute_overall_iou(pred, target, num_classes):
    ...
    for shape_idx in range(pred.size(0)):
        ...
        for part in range(num_classes):
            I = np.sum(np.logical_and(pred_np[shape_idx].max(
                1) == part, target_np[shape_idx] == part))
            U = np.sum(np.logical_or(pred_np[shape_idx].max(
                1) == part, target_np[shape_idx] == part))
            if U == 0:
                iou = 1  # If the union of groundtruth and prediction points is empty, then count part IoU as 1
            else:
...
def test_partseg(model, loader, catdict, num_classes=50, forpointnet2=False):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        ...
        points, label, target, norm_plt = points.cuda(
        ), label.squeeze().cuda(), target.cuda(), norm_plt.cuda()
        if forpointnet2:
...
def test_semantic_kitti(model, loader, catdict, num_classes=13, pointnet2=False):
    iou_tabel = np.zeros((len(catdict), 3))
    metrics = defaultdict(lambda: list())
    hist_acc = []
    for batch_id, data in tqdm(enumerate(loader), total=len(loader), smoothing=0.9):
        points = data['velo']
        target = data['label']
        batchsize, num_point, _ = points.shape
        points, target = Variable(
            points.float()), Variable(target.float().long())
        points = points.transpose(2, 1)
        points, target = points.cuda(), target.cuda()
        if pointnet2:
            pred = model(points[:, :3, :], points[:, 3:, :])
...
def plot_loss_curve(history, n_points_avg, n_points_plot, save_dir):
    ...
    plt.savefig(save_dir + '/' +
                str(datetime.datetime.now().strftime('%Y-%m-%d %H-%M')) + '_total_result.png')
    plt.close()
...
def plot_acc_curve(total_train_acc, total_test_acc, save_dir):
    ...
    plt.savefig(save_dir + '/' +
                str(datetime.datetime.now().strftime('%Y-%m-%d %H-%M'))+'_total_acc.png')
    plt.close()
