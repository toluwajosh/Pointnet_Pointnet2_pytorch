...
class PointNet2PartSeg(nn.Module):  # TODO part segmentation tasks
    def __init__(self, num_classes):
        ...
        self.sa1 = PointNetSetAbstraction(
            npoint=512, radius=0.2, nsample=64, in_channel=3, mlp=[64, 64, 128], group_all=False)
        self.sa2 = PointNetSetAbstraction(
            npoint=128, radius=0.4, nsample=64, in_channel=128 + 3, mlp=[128, 128, 256], group_all=False)
        self.sa3 = PointNetSetAbstraction(
            npoint=None, radius=None, nsample=None, in_channel=256 + 3, mlp=[256, 512, 1024], group_all=True)
...
class PointNet2PartSeg_msg_one_hot(nn.Module):
    def __init__(self, num_classes):
        ...
        self.sa1 = PointNetSetAbstractionMsg(512, [0.1, 0.2, 0.4], [
                                             32, 64, 128], 0+3, [[32, 32, 64], [64, 64, 128], [64, 96, 128]])
    ...
    def forward(self, xyz, norm_plt, cls_label):
        ...
        l0_points = self.fp1(l0_xyz, l1_xyz, torch.cat(
            [cls_label_one_hot, l0_xyz, l0_points], 1), l1_points)
        # FC layers
        feat = F.relu(self.bn1(self.conv1(l0_points)))
        x = self.drop1(feat)
        x = self.conv2(x)
...
class PointNet2SemSeg(nn.Module):
    def __init__(self, num_classes):
        ...
        self.sa1 = PointNetSetAbstraction(
            # 1024, 0.1, 32, 6 + 3, [32, 32, 64], False)
            1024, 0.1, 32, 4, [32, 32, 64], False)
        self.sa2 = PointNetSetAbstraction(
